...
angular.module('myjdWebextensionApp')
  .controller('BackgroundCtrl', ['$scope', '$timeout', '$q', 'Rc2Service', 'BrowserService', 'PopupIconService', 'CnlService', 'StorageService', 'ClipboardHistoryService', 'StringUtilsService', 'myjdClientFactory', 'myjdDeviceClientFactory', 'ExtensionMessagingService', 'PopupCandidatesService', 'ExtensionI18nService',
    function ($scope, $timeout, $q, Rc2Service, BrowserService, PopupIconService, cnlService, StorageService, ClipboardHistoryService, StringUtilsService, myjdClientFactory, myjdDeviceClientFactory, ExtensionMessagingService, PopupCandidatesService, ExtensionI18nService) {
      ...
      // Replicates AddLinksController's CnL packaging: wraps the captured
      // form data in a dummycnl.jdownloader.org link (JDownloader decodes the
      // hex fragment and decrypts crypted/jk server-side) and adds it directly
      // to the connected device via the https cloud API.
      ...
      ExtensionMessagingService.addListener("myjd-cnl-capture", "captured", function (request, sender, sendResponse) {
        try {
          if (request.data && validCnlCapture(request.data) && sender && sender.tab && sender.tab.id !== undefined) {
            ...
            // so an in-page toolbar (with its send countdown) gets killed by the
      ...
      ExtensionMessagingService.addListener("myjd-toolbar", "login", function (request, sender, sendResponse) {
        if (request.data && request.data.credentials) {
          myjdClientFactory.get().connect(request.data.credentials).then(function () {
            ...
            console.error("[MyJD] login connect FAILED:", error, JSON.stringify(error));
            sendResponse({ data: false, error: error });
            ExtensionMessagingService.sendMessage("myjd-toolbar", "session-change", {
              isLoggedIn: false
            });
      ...
      // Safari compatibility: webRequest is observational only. Calling
      // addListener with "blocking"/"requestBody" throws on Safari, which would
      // abort BackgroundController init and break the whole popup/device UI.
      // safeAddWebRequestListener degrades gracefully: it tries the blocking
      // registration first (Chrome/Firefox) and, on failure, retries as a
      // plain observer so the rest of the extension keeps working.
      let safeAddWebRequestListener = function (event, listener, filter, extraInfo) {
        try {
          ...
          try {
            ...
            console.warn("webRequest listener could not be registered (Safari has no blocking webRequest):", e2 && e2.message);
          }
          return false;
        }
      };
      ...
      let addRequestHeaderInterceptor = function () {
        if (!chrome.webRequest.onBeforeSendHeaders.hasListener(interceptHeaders)) {
          ...
          safeAddWebRequestListener(chrome.webRequest.onBeforeSendHeaders,
            interceptHeaders, {
            urls: ["<all_urls>"]
      ...
      let addCnlInterceptor = function () {
        if (!chrome.webRequest.onBeforeRequest.hasListener(interceptRequest)) {
          chrome.webRequest.onBeforeRequest.removeListener(interceptRequest);
          safeAddWebRequestListener(chrome.webRequest.onBeforeRequest,
            interceptRequest, {
            urls: ["<all_urls>"]
